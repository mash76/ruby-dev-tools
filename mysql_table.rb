...
def main()
    ...
    $table_name = p[:table_name].to_s


    limit = 200

    if $conn_name == "" || $table_name == ""
        out sRed("param need : connection/table")
        return
    end

    # 件数
    ct_row_all = sql2hash("select count(*) ct FROM " + $table_name , $conn_name,false)

    out sBlue($conn_name) + br
    out s150(sBlue($table_name)) + spc + s150(ct_row_all[0]['ct'].to_s) + br
    ...
    # fk_recs = sql2hash(sql_fk_parent($conn_name,$table_name), $conn_name)
    # out hash2html(fk_recs)

    out br + sBG("data") + spc + ct_row_all[0]['ct'].to_s
    out hash2html_mysql_data($table_name  ,p )
end
...
def table_info()
    table_info = sql2hash("select
        ...
        from information_schema.TABLES where TABLE_SCHEMA='" + $conn_name + "' AND TABLE_NAME='" + $table_name + "'", $conn_name)
        out hash2html(table_info)
end

def showFKS(table_name,use_tables,depth,max_depth)
    if depth > max_depth
        out sRed("depth-limit! max-depth " + max_depth.to_s + " now ") + depth.to_s + br
        return
    end

    if use_tables.include?(table_name)
       # out "already " + sSilver(table_name) + br
        return
    else
        out br + "check " + sRed(table_name) + spc + depth.to_s +  br
    end

    use_tables << table_name
    fk_child_recs = sql2hash(sql_fk_child($conn_name,table_name), $conn_name,false)
    fk_childs = {}
    fk_child_recs.each { |row| fk_childs[row['COLUMN_NAME']] = row }
    fk_parent_recs = sql2hash(sql_fk_parent($conn_name,table_name), $conn_name,false)
    fk_parents = {}
    fk_parent_recs.each { |row| fk_parents[row['REFERENCED_COLUMN_NAME']] = row }

    cols2 = sql2hash("SELECT '' FK_P,COLUMN_NAME ,'' FK_C
            from information_schema.COLUMNS WHERE TABLE_SCHEMA='" + $conn_name + "' and TABLE_NAME='" + table_name + "'", $conn_name,false)
    cols2.each do |row|
...
def fk_map()
        ...
        out sBlue("FK map") + br
        use_tables = []
        showFKS($table_name, use_tables,0,1)
        ...
        columns_recs = sql2hash("SELECT
            ...
            WHERE TABLE_SCHEMA='" + $conn_name + "' AND TABLE_NAME='" + $table_name + "'
            ORDER BY ORDINAL_POSITION ASC", $conn_name)

        # fkの親と子
        columns_recs.each do  | row |
            col_name = row['COLUMN_NAME']
            if fk_parents.key?(col_name)
                row['FK_PARENT'] = a_tag(fk_parents[col_name]['REFERENCED_TABLE_NAME'] + "." + fk_parents[col_name]['REFERENCED_COLUMN_NAME'],"?conn_name=" + $conn_name + "&table_name=" + fk_parents[col_name]['REFERENCED_TABLE_NAME'])
            end
            if fk_childs.key?(col_name)
                row['FK_CHILD'] = a_tag(fk_childs[col_name]['TABLE_NAME'] + "." + fk_childs[col_name]['COLUMN_NAME'],"?conn_name=" + $conn_name + "&table_name=" + fk_childs[col_name]['TABLE_NAME'])
            end
        end
...
def hash2html_mysql_data2(table_name ,p ,p_class="" )

end

# 絞り込み sort 指定テーブルを一覧する sortもする
def hash2html_mysql_data(table_name ,p ,p_class="" )
    ...
    col_datatypes = get_col_datatypes($conn_name,$table_name)
    ...
    sql_base = "select * from " + table_name
    col_infos = sql2hash(sql_base + " limit 1",$conn_name)
    sql = sql_base
    ...
    html << '<form id="f1" action="?conn_name=' + $conn_name + '&table_name=' + table_name + '" >'
    html << i_hidden("conn_name",$conn_name)
    html << i_hidden("table_name",table_name)
...
def get_col_datatypes(conn_name,table_name)
    ret = {}
    col_recs = sql2hash("SELECT
        COLUMN_NAME,DATA_TYPE
    FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA='" + conn_name + "' AND TABLE_NAME='" + table_name + "' ", conn_name,false)
...
def sql_fk_parent(conn_name,table_name)
...
def sql_fk_child(conn_name,table_name)
